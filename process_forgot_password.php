...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['email'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $resetLink = "http://localhost/booking-hotel/reset_password.php?token=$token";
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = 'Pha Chom Dao Resort เปลี่ยนรหัสผ่าน';
            $mail->Body = "กดเพื่อเปลี่ยนรหัสผ่าน <a href='$resetLink'>$resetLink</a>";
        
            $mail->send();
            echo "<script>alert('ลิงค์รีเซ็ตส่งไปที่อีเมล์ของคุณแล้ว'); window.location.href = 'login.php';</script>";
        } catch (Exception $e) {
